...
public class Patron extends Thread {
	...
	public void run() {
		try {
			...
			long waitTime = endTime - responseStart; // total time actually in queue

			for (DrinkOrder drink : drinksOrder) {
				waitTime -= drink.getExecutionTime(); // subtract time it takes for drink to be worked on
			}

			SchedulingSimulation.totalResponseTime += responseTime;
			SchedulingSimulation.totalWaitTime += waitTime;
			SchedulingSimulation.totalTurnaroundTime += totalTime;

			// writeToFile( String.format("%d,%d,%d\n",ID,arrivalTime,totalTime)); //
			// provided write to file code
			writeToFile(String.format("%d,%d,%d,%d\n", ID, totalTime, waitTime, responseTime)); // patron ID, turnaround
																								// time, waiting time,
																								// response time
